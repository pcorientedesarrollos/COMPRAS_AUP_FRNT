...
export class AcopiadorService {
  ...
  // CSF (Constancia de Situación Fiscal) methods
  uploadCSF(idDatosFiscales: number, file: File): Observable<ApiResponse> {
    const formData = new FormData();
    formData.append('csf', file);
    
    return this.http.post<ApiResponse>(`http://localhost:3000/api/datosfiscales/${idDatosFiscales}/csf`, formData);
  }

  downloadCSF(idDatosFiscales: number): Observable<Blob> {
    return this.http.get(`http://localhost:3000/api/datosfiscales/${idDatosFiscales}/csf`, { 
      responseType: 'blob' 
    });
  }

  getCSFUrl(idDatosFiscales: number): string {
    return `http://localhost:3000/api/datosfiscales/${idDatosFiscales}/csf`;
  }

  deleteCSF(idDatosFiscales: number): Observable<ApiResponse> {
    return this.http.delete<ApiResponse>(`http://localhost:3000/api/datosfiscales/${idDatosFiscales}/csf`);
  }
}
